...
- 生成区分自动/人工的详细签到报告（`gen-report.py`，按当前日期输出 `signin-report-YYYY-MM-DD.html`）
- 将报告推送到飞书 Webhook
  - 走 `signin-batch.ps1` 主流程：末尾自动调用 `Send-FeishuSummary`，无需额外操作
  - 走续跑流程（`rerun-remaining.ps1`，用于防止后台任务跨 turn 被回收导致结果全丢）：**必须**在 4 块全部跑完后单独执行 `.\push-cumulative.ps1` 补推——该路径本身不含推送，否则收不到飞书通知
- 清理工作目录中的临时调试文件
